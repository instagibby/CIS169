...
namespace M06_Davis_HospitalCharges
{
    public partial class HospitalCharges : Form
    {
        ...
        public int CalcStayCharges(string t)
        {
             
            //  Parses for days and validates its atleast one day
            if (int.TryParse(t, out int daysSpentValue) && daysSpentValue >= 1)
            {
            ...
            else
            {
                ...
                throw new ArgumentException("Please enter a valid number into the days box.");
            }

            return baseCostStay;
        }

        public double CalcMiscCharges(string lftb, string stb, string mtb, string prtb)
        {
            miscFees = 0;

            //  Parses for medication charges and validates its not negative
            if (double.TryParse(mtb, out double medicationCharge) && medicationCharge >= 0)
            {
            ...
            else
            {
                ...
                throw new ArgumentException("Please enter a valid number into the medication box.");
            }

            //  Parses for surgical charges and validates its not negative
            if (double.TryParse(stb, out double surgicalCharge) && surgicalCharge >= 0)
            {
            ...
            else
            {
                ...
                throw new ArgumentException("Please enter a valid number into the surgical box.");
            }

            // Parses for lab fees and validates its not negative
            if (double.TryParse(lftb, out double labFees) && labFees >= 0)
            {
                miscFees += labFees;
            }
            else
            {
                ...
                throw new ArgumentException("Please enter a valid number into the lab fees box.");
            }

            //  Parses for physical rehab charges and validates its not negative
            if (double.TryParse(prtb, out double physicalRehabCharges) && physicalRehabCharges >= 0)
            {
                miscFees += physicalRehabCharges;
            }
            else
            {
                errorMsg += "Please enter a valid number into the rehab box. \n";
                throw new ArgumentException("Please enter a valid number into the rehab box.");
            }
        ...
        private double CalcTotalCharges()
        {
            ...
            return CalcStayCharges(daysSpentTextBox.Text) 
                + CalcMiscCharges(labFeesTextBox.Text, surgicalTextBox.Text,
                medicationTextBox.Text, physicalRehabTextBox.Text);
        }
        ...
        private void calculateBtn_Click(object sender, EventArgs e) 
        {
            ...
            else
            {
                ...
                baseChargeValueLabel.Text = CalcStayCharges(daysSpentTextBox.Text).ToString("c");
                miscChargeValueLabel.Text = CalcMiscCharges(labFeesTextBox.Text, surgicalTextBox.Text,
                medicationTextBox.Text, physicalRehabTextBox.Text).ToString("c");
